...
export const createVentasSeguro = async (req, res) => {
    const { fechaVenta, tipoVehiculo, placaVehiculo, cedulaCliente, valorVenta, cliente } = req.body;
    ...
    if (nuevaFechaVenta.getMonth() > fechaHoyCero.getMonth() && nuevaFechaVenta.getFullYear() <= fechaHoyCero.getFullYear()) {
        return res.status(400).json({ message: 'El mes ingresado no es adecuado, por favor ingrese el mes en el que se esta registrando la venta' });
    }
    try {
        const vehicleExist = await NuevaVentaSeguro.findOne({ placaVehiculo })
        if (vehicleExist) {
            return res.status(400).json({ message: 'El vehiculo que intenta ingresar ya cuenta con un seguro activo' });
        }
        ...
        if (!customerFound) {
            res.status(409).json({ message: 'El cliente ingresado no esta registrado, por favor creelo antes de intentar registrar una venta a este cliente' });
        }
        if (customerFound) {
            ...
            const saleCreated = await NuevaVentaSeguro.create({
                anio: anio, mes: mes, dia: dia, fechaExpiracion: fechaExpiracion, tipoVehiculo: tipoVehiculo, placeVehiculo: placaVehiculo, valorVenta: valorVenta, cedulaCliente: customerFound[0].cedula
            });
            const clienteEncontrado = await NuevoClienteSeguro.findOne({ cedula: cliente });
...
const ventaVencida = async (req, res) => {
    ...
    try {
        ventas = await NuevaVentaSeguro.updateMany({ mes: pastMonth }, { editable: false });
        return ventas;
    } catch (error) {
        return error;
    }
